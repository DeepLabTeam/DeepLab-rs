...
pub struct Operation {
    ...
    pub num_inputs: u64,
    pub num_outputs: u64,
    pub build: Box<Fn(&matrix::Context, &mut dl::Graph, &VarStore, &[Option<VarIndex>], &[VarIndex]) -> dl::NodeIndex>,
}

impl Operation {
    pub fn new<F>(name: String, num_inputs: u64, num_outputs: u64, build: F) -> Self
        where F: Fn(&matrix::Context, &mut dl::Graph, &VarStore,
                    &[Option<VarIndex>], &[VarIndex]) -> dl::NodeIndex + 'static {
        Operation {
            name: name,
